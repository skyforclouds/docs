...
## Example 4: Deploying a GPU-Accelerated Job Processing Daemon Application

Suppose you have a custom job processor image (`my-gpu-job-processor:latest`) that fetches jobs from a queue, processes them using the GPU, stores results, and repeats. To scale this processor quickly, you can simply provision new machines and this application will automatically start on those newly added machines.

First, pull and push your custom image to your SoraNova registry:

```shell
docker pull <your-job-processor-image>
sora image push <your-job-processor-image> my-gpu-job-processor:latest
```

Then, create an application recipe, e.g. `my-gpu-job-processor.hcl`:
...
application {
  ...
  service "job-processor" {
    ...
    task_group "job-processor" {
      name  = "gpu-job-processor-task-group"
      count = 1
      ...
      task {
        ...
        driver = "docker"
        config {
          image = "my-gpu-job-processor:latest"
          # Optionally add args or env for queue endpoints, credentials, etc.
        }
...
> This service will run on nodes with available GPUs and process jobs as they arrive in the queue. Make sure your image implements the required logic for job fetching, processing, and result storage.

To deploy:

```shell
sora recipe seed my-gpu-job-processor.hcl
sora recipe list
sora recipe deploy <recipe-slug>
```
